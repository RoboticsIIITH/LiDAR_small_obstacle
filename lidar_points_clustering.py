...
def main(draw=True, path=False, paths=None):
    ...
    # ring_id = 4
    # # retrieving valid x, y positions that have depth
    # x_1_global, y_1_global = np.where(ring == ring_id)
    # x_1_road = []
    # y_1_road = []
    # # retrieving valid x,y positions within the road polygon
    # for i in range(len(x_1_global)):
    #     if x_1_global[i] < max_road_x and x_1_global[i]>min_road_x and \
    #     y_1_global[i]<max_road_y and y_1_global[i]>min_road_y and gt[x_1_global[i], \
    #                                                               y_1_global[i]]!=0:
    #         x_1_road.append(x_1_global[i])
    #         y_1_road.append(y_1_global[i])
    ...
    for ring in unique_road_rings:
        ...
        feature_vector = [[x[i], y[i], value[i]] for i in relevant_indices]
        ring_depths = value[relevant_indices]
        ring_depths = np.asarray(ring_depths)
        ring_depths = ring_depths[:, np.newaxis]
        preds = custom_fit(x[relevant_indices][:, np.newaxis], ring_depths)
